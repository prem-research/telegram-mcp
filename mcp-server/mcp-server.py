...
async def get_unread_entities() -> dict[dict]:
    return get('http://127.0.0.1:8080/get_unread_chats').json()
...
async def get_unread_messages(id: int = None) -> list[dict]:
    return get(f'http://127.0.0.1:8080/get_unread_messages/{id}').json()

@mcp.tool(
    name='get_messages',
    description='Get messages limited by a count from an entity'
)
